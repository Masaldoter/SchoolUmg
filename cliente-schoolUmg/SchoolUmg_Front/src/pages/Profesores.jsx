...
export function Profesores() {
  ...
  const handleModalSubmit = (e) => {
    ...
    if (currentProfesor.pass && currentProfesor.pass.length < 4) {
      Swal.fire("Error", "La contraseña debe tener al menos 4 caracteres", "warning");
      return;
    }
    if (currentProfesor.email && !/^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/.test(currentProfesor.email)) {
              ...
              <div className="modal-body">
                {columnasDisponibles.map(col => (
                  <div key={col.key} className="form-check">
                    ...
                    />
                    <label className="form-check-label" htmlFor={`check-${col.key}`}>{col.label}</label>
                  </div>
                ))}
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-success" onClick={() => handleExport("excel")}>Exportar Excel</button>
                <button type="button" className="btn btn-danger" onClick={() => handleExport("pdf")}>Exportar PDF</button>
              </div>
            </div>
          </div>
        </div>
